...
def input_dataget_files(file_dir):
    cats = []
    label_cats = []
    dogs = []
    label_dogs = []
    #获取图片，然后根据图片名，添加猫狗label
    #再将image和label放入数组中
...
#测试 get_files
# imgs , label = input_dataget_files('./data/')
# for i in imgs:
#     print("img:",i)
...
def input_dataget_batch(image,label,image_W,image_H,batch_size,capacity):
    # 转换数据为 ts 能识别的格式
...
# image_list, label_list = input_dataget_files(train_dir)
# image_batch, label_batch = input_dataget_batch(image_list, label_list, IMG_W, IMG_H, BATCH_SIZE, CAPACITY)
